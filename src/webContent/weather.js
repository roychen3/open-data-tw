...
import {switchDay, filterDatas, SelectOption, WebMessage} from '../community/community';
...
function dataToWeather(datas, condition) {
    ...
    for (let i = 0; i < datas[weatherDataSite].weatherElement.length; i++) {
        ...
        for (let j = 0; j < datas[weatherDataSite].weatherElement[i].time.length; j++) {
            infoStartTime = datas[weatherDataSite].weatherElement[i].time[j].startTime;
            ...
            if (infoType === "Wx") {
                // 天氣描述
                weatherDescription = datas[weatherDataSite].weatherElement[i].time[j].parameter.parameterName;
                weatherObj.info[j].weatherDescription = weatherDescription;
            }
            else if (infoType === "MaxT") {
                // 最高溫度
                degree = datas[weatherDataSite].weatherElement[i].time[j].parameter.parameterName + "℃";
                ...
                // 降雨機率
                rainProbability = datas[weatherDataSite].weatherElement[i].time[j].parameter.parameterName + "%";
...
class WeatherCard extends React.Component {
    ...
    render() {
        ...
        return (<div className="weather-card">
            <div className="weather-time">{date}<br />
                <p className="start-time">{startTime}</p>
                <p className="end-time"> - {endTime}</p>
            </div>
            <div className="weather-description">{this.props.dayInfo.weatherDescription}</div>
            <div className="weather-temperature">{this.props.dayInfo.minTemperature} ~ {this.props.dayInfo.maxTemperature}</div>
            <div className="rain-probability">降雨機率：{this.props.dayInfo.rainProbability}</div>
        </div>
        );
    }
}
...
class Weather extends React.Component {
    ...
    getWeatherDatas() {
        ...
        let url = "https://opendata.cwb.gov.tw/fileapi/v1/opendataapi/F-C0032-001?Authorization=rdec-key-123-45678-011121314&format=JSON";
    ...
    render() {
        ...
        if (this.state.isLoading) {
            ...
            if (this.state.isError) {
                ...
                let weatherData = filterDatas(this.state.webDatas, this.state.city, dataToWeather);
                const weatherAllCard = weatherData.info.map((dayInfo, i) => <WeatherCard key={i} dayInfo={dayInfo} />);
                weather = (<div>
                    <SelectOption optionDatas={this.state.webDatas} defaultSelected={this.state.city} onSelecOptionChange={this.onSelecOptionChange} />
